...
PATH = 'data/clothodetail'

class ClothoDetailLoader(BaseHypnetLoader):
    def __init__(self, tokenizer, train_args, model_name, is_instruct, extract_features=False):
        ...
        self.max_new_tokens = 88
        self._init_datasets()
        self._init_prefix_emb_dict()
        self.dataset_name = 'clothodetail'

    def _init_prefix_emb_dict(self):
        with open(osp.join('data/prefixes/audio_inst.pkl'), 'rb') as f:
    ...
    def _init_split(self, split):
        ...
        for idx, (key, value) in enumerate(split_set_dict.items()):
            item = dict(audioid=key, caption=value['caption'], emb=value['emb'])
    ...
    def _init_datasets(self):
        ...
        if self.debug:
            ...
            if dataset_size > len(train_set):
                dataset_size = min(dataset_size, len(train_set))
                print(f"Tried using {dataset_size} samples, dataset is limited to {len(train_set)} samples, using full dataset")
